...
export class HomeComponent extends BaseComponent {
  ...
  init() {
    ...
    const spursRelEl = this.shadowRoot?.getElementById('spurs-relegated-home');
    ...
    if (spursRelEl) spursRelEl.innerHTML = spursCrest;
    if (arsenalEl) arsenalEl.innerHTML = arsenalCrest;
    if (spursSurvEl) spursSurvEl.innerHTML = spursCrest;
    if (cityEl) {
      cityEl.innerHTML = `<svg viewBox="0 0 80 80"><circle cx="40" cy="40" r="38" fill="#6CABDD" stroke="#ffffff" stroke-width="2"/><text x="50%" y="55%" text-anchor="middle" fill="white" font-weight="900" font-size="20">MCFC</text></svg>`;
    }
